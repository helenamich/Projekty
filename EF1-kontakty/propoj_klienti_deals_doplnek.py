#!/usr/bin/env python3
"""
Propojí Klienty s Deals - doplněk podle názvu firmy.
"""
...
def main():
    ...
    while True:
        ...
        for rec in data.get("records", []):
            ...
            firma = fields.get("Firma", "").strip()
            firma_norm = normalize_company(firma)
            current_deals = set(fields.get("Deals", []))
    ...
    if not klienti_to_update:
        print("\n✅ Všichni klienti už jsou propojení!")
        return
    ...
    print(f"\n✅ Propojeno {updated} klientů s Deals - doplněk!")
